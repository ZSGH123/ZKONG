...
var swfobject = function() {
	...
	function f() {
		...
		try {
			...
			ac = h.getElementsByTagName("body")[0].appendChild(ad);ac.parentNode.removeChild(ac);
			ac = null;
			ad = null
		} catch (ae) {
			return
		}
		L = true;var aa = X.length;
	...
	function Y() {
		...
		var ae = C(r);
		ae.setAttribute("style", "visibility: hidden;");ae.setAttribute("type", q);var ad = aa.appendChild(ae);
		if (ad) {
			var ac = 0;
			(function ab() {
	...
	function R(ad, ae, aa, ac) {
		...
		if (ah) {
			...
			if (O.ie && ah.readyState != 4) {
				var ab = C("div");
				aa += "SWFObjectNew";ab.setAttribute("id", aa);ah.parentNode.insertBefore(ab, ah);
				ah.style.display = "none";y(ah)
			}
			u(ad, ae, aa)
		}
	}
	...
	function u(ai, ag, ab) {
		...
		if (ad) {
			...
			if (O.ie) {
				...
				ac.setAttribute("type", q);ac.setAttribute("data", ai.data)
			}
	...
	function v(af, ab, ag, ae) {
		...
		if (!m || G != aa) {
			var ac = C("style");
			ac.setAttribute("type", "text/css");ac.setAttribute("media", aa);
			m = ad.appendChild(ac);
			...
			if (O.w3 && !(O.wk && O.wk < 312) && af && al && ai && ak && ab) {
				w(ac, false);M(function() {
					...
					if (F(ab)) {
						...
						if (ae && A()) {
							an.data = ae;R(an, ar, al, ag);return
						} else {
							w(ac, true)
						}
					}
