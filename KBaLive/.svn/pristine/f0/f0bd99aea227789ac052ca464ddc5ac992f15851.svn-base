			...
			<div class="col-sm-8 col-sm-offset-2">
				<div class="panel panel-warning">
					<div class="panel-heading">
						<h3 class="panel-title" style="font-size: 25px;">修改主播等级</h3>
					</div>
					<div class="panel-body">
						<div class="jumbotron" style="padding: 20px;">
                            ...
                            <div id="anchorLevelId" style="display:none;" >${updateOfAnchorLevel.anchorLevelId}</div>
						<form id="findform" method="post" class="form-horizontal">
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
									<input type="text" class="form-control" id="anchorLevelName"
										name="anchorLevelName" value="${updateOfAnchorLevel.anchorLevelName}" />
								</div>
							</div>
                            <div class="form-group">
								...
								<div class="col-sm-5">
									<input type="text" class="form-control" id="liveMinTotalTime"
										name="liveMinTotalTime" value="${updateOfAnchorLevel.liveMinTotalTime}" />
								</div>
							</div>
							 <div class="form-group">
								...
								<div class="col-sm-5">
									<input type="text" class="form-control" id="liveMaxTotalTime"
										name="liveMaxTotalTime" value="${updateOfAnchorLevel.liveMaxTotalTime}"/>
								</div>
							</div>
                            ...
                            <div class="form-group">
								...
								<div class="col-sm-5">
									...
									id="anchorLevelRemark" name="anchorLevelRemark">${updateOfAnchorLevel.anchorLevelRemark}</textarea>
								</div>
							</div>
		...
		$(function() {
			$("#findform").validate({
			 ...
			 function refreshParent(){
	            var id=$("#anchorLevelId").html();
	            var name=$("#anchorLevelName").val();
                 var pwd=$("#liveMinTotalTime").val();
                 var pos=$("#liveMaxTotalTime").val();
                 var remark=$("#anchorLevelRemark").val();
                
                 $.ajax({
                     url:"backStageAnchorLevelAction.do?command=updateAnchorLevel",
                     type:"post",
                     dataType:"text",
                     async:false,
                     data:{
                          anchorLevelId:id,anchorLevelName:name,liveMinTotalTime:pwd,liveMaxTotalTime:pos,anchorLevelRemark:remark
                     },
	      ...
	      function closeWindow(){
	        $.post("backStageAnchorLevelAction.do?command=quXiao",function(data){
	            if(data){
	               window.location.href="backStageAnchorLevelAction.do";      
	            }else{
	               confirm("关闭失败了");
