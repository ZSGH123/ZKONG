...
public class AnchorInfoDao extends AbstractBaseDao implements IAnchorInfoDao,ResultSetHandler<List<AnchorInfo>>{
	...
	private static AnchorInfoDao anchorInfoDao;
	
	private AnchorInfoDao(){
		
	}
	
	public synchronized static AnchorInfoDao getInstance(){
		if(anchorInfoDao==null){
			anchorInfoDao=new AnchorInfoDao();
		}
	 ...
	 * @throws SQLException
	 */
	public AnchorInfo querySingleByUserId(AnchorInfo entity) throws SQLException{
		String sql=null;
		Object[] params=null;
		if(entity==null)return null;
		sql=this.parseSqlStatement.getSql(NameSpace.ANCHORINFO,ISqlCommand.SELECT_BY_USERID);
		params=new Object[]{entity.getUserId()};
		try {
