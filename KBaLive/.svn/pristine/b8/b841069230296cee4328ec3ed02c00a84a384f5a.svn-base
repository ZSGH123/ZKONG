...
public class InvestRecordDao extends AbstractBaseDao implements IInvestRecordDao, ResultSetHandler<List<InvestRecord>> {
	...
	public List<InvestRecord> queryAll(InvestRecord entity) throws SQLException {
		...
		Object[] args=null;
		if(entity==null){
			sql=this.parseSqlStatement.getSql(NameSpace.INVESTRECORD, ISqlCommand.SELECT_ALL);
		}else{
			sql=this.parseSqlStatement.getSql(NameSpace.INVESTRECORD, ISqlCommand.SELECT_BY_USERID);
			args=new Object[]{entity.getUserId()};
		}
		try {
			investRecords=this.queryRunner.query(sql, this,args);
