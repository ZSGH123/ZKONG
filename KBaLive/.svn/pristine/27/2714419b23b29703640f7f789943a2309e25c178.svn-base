...
public class ReportMessageDao extends AbstractBaseDao implements IReportMessageDao,ResultSetHandler<List<ReportMessage>>{
	...
	private static ReportMessageDao reportMessageDao;
	
	private ReportMessageDao(){
		
	}
	
	public synchronized static ReportMessageDao getInstance(){
		if(reportMessageDao==null){
			reportMessageDao=new ReportMessageDao();
		}
	...
	public List<ReportMessage> queryAll(ReportMessage entity) throws SQLException {
		...
		Object[] args=null;
		if(entity==null){
			sql=this.parseSqlStatement.getSql(NameSpace.REPORTMESSAGE, ISqlCommand.SELECT_ALL);
		}else{
			sql=this.parseSqlStatement.getSql(NameSpace.REPORTMESSAGE, ISqlCommand.SELECT_BY_USERID);
			args=new Object[]{entity.getUserId()};
		}
		try {
			reportMessages=this.queryRunner.query(sql, this,args);
	...
	@Override
	public List<ReportMessage> queryByUserId(ReportMessage entity) throws SQLException {
		List<ReportMessage> reportMessages=null;
		String sql=this.parseSqlStatement.getSql(NameSpace.REPORTMESSAGE, ISqlCommand.SELECT_BY_USERID);
		try {
