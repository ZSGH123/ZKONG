				...
				<c:forEach items="${allLiveLiveContentInfos}" var="item" >
				    ...
				    <ul class="leftul1">
				       <c:forEach items="${liveContentListItem}" var="listItem" >
					    <li>
						  <a href="LiveContentAction.do?liveContentId=${listItem.liveContentId}">${listItem.liveContentName}</a>
					    </li>
					   </c:forEach>
					   <li class="liquanbu" style="color: #f60;"><a href="liveIndexAction.do?command=toLiveTypeIndex&liveTypeId=${liveTypeItem.liveTypeId}">全部>></a></li>
				    </ul>
				</c:forEach>
