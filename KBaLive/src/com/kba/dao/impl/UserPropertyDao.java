...
public class UserPropertyDao extends AbstractBaseDao implements IUserPropertyDao,ResultSetHandler<List<UserProperty>>{
	 ...
	 */
	@Override
	public List<UserProperty> querySinglebyid(UserProperty entity) throws SQLException {
		String sql=null;
		Object[] params=null;
		...
		try {
			List<UserProperty> userPropertys=this.queryRunner.query(sql, this, params);
			return userPropertys != null && userPropertys.size() > 0 ? userPropertys : null;
		}catch (SQLException e) {
			throw e;
		}catch (Exception e) {
			logger.error("error",e);
	...
	public List<UserProperty> handle(ResultSet rs) throws SQLException {
		...
		while(rs.next()){
			...
			userProperty.setkNumber(Integer.parseInt(rs.getString("K_NUMBER")));
