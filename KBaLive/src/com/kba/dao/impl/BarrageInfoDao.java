...
public class BarrageInfoDao extends AbstractBaseDao implements IBarrageInfoDao,ResultSetHandler<List<BarrageInfo>>{
	...
	private static BarrageInfoDao barrageInfoDao;
	
	private BarrageInfoDao(){
		
	}
	
	public synchronized static BarrageInfoDao getInstance(){
		if(barrageInfoDao==null){
			barrageInfoDao=new BarrageInfoDao();
		}
		return barrageInfoDao;
	}
	/**
	 * 查询所有的弹幕
	 */
	...
	public List<BarrageInfo> handle(ResultSet rs) throws SQLException {
		...
		while(rs.next()){
			...
			barrageInfo.setNowVideoDuration(Integer.parseInt(rs.getString("NOW_VIDEO_DURATION")));
