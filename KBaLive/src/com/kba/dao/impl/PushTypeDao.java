...
public class PushTypeDao extends AbstractBaseDao implements IPushTypeDao,ResultSetHandler<List<PushType>>{
	...
	private static PushTypeDao pushTypeDao;
	
	private PushTypeDao(){
		
	}
	
	public synchronized static PushTypeDao getInstance(){
		if(pushTypeDao==null){
			pushTypeDao=new PushTypeDao();
		}
		return pushTypeDao;
	}
	/**
	 ...
	 * @throws SQLException
	 */
	public PushType querySingleByName(PushType entity) throws SQLException {
		String sql=null;
		Object[] params=null;
		
		if(entity==null)return null;
		
		sql=this.parseSqlStatement.getSql(NameSpace.PUSHTYPE,ISqlCommand.SELECT_BY_NAME);
		
		params=new Object[]{entity.getPushName()};
