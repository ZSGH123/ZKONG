...
public class UserGiftRecordDao extends AbstractBaseDao implements IUserGiftRecordDao,ResultSetHandler<List<UserGiftRecord>>{
	...
	@Override
	public List<UserGiftRecord> querySinglebyid(UserGiftRecord entity) throws SQLException {
		String sql=null;
		Object[] params=null;
		...
		try {
			List<UserGiftRecord> userGiftRecords=this.queryRunner.query(sql, this, params);
			return userGiftRecords != null && userGiftRecords.size() > 0 ? userGiftRecords : null;
		}catch (SQLException e) {
			throw e;
		}catch (Exception e) {
			logger.error("error",e);
	...
	public List<UserGiftRecord> handle(ResultSet rs) throws SQLException {
		...
		while(rs.next()){
			...
			userGiftRecord.setGiftNumber(Integer.parseInt(rs.getString("GIFT_NUMBER")));
	...
	@Override
	public List<UserGiftRecord> queryByUserId(UserGiftRecord entity) throws SQLException {
