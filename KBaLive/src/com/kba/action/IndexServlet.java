...
public class IndexServlet extends KBaBaseServlet {
	 ...
	 * @throws IOException
	 */
	protected void toSearch(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		HttpSession session = this.getSession();
		String content=req.getParameter("content");
		List<LiveRoomDetailInfo> allPushLiveRoomDetailInfos=null;
		try {
			allPushLiveRoomDetailInfos=indexService.selectAllMoHu(content);
			session.setAttribute("allPushLiveRoomDetailInfos", allPushLiveRoomDetailInfos);	
			req.getRequestDispatcher("serchresult.jsp").forward(req, resp);	
		} catch (KBaException e) {
			this.toError(e, resp);
		}
	}
