...
public class LiveRoomService implements ILiveRoomService {
	...
	public List<LiveRoom> selectByLiveContentID(LiveRoom liveRoom) throws KBaException {
		return selectByLiveContentID(liveRoom, -1);
	}
	 /**
     * 通过直播内容查找相应数量num房间
     */
	public List<LiveRoom> selectByLiveContentID(LiveRoom liveRoom,int num) throws KBaException {
		
		List<LiveRoom> liveRooms = null;
		try {
			liveRooms = liveRoomDao.queryByLiveContentID(liveRoom);
			if(num==-1) return liveRooms;
			if(num>liveRooms.size()){
				return liveRooms;
			}else {
				return liveRooms.subList(0, num);
			}
		} catch (SQLException e) {
			throw new KBaException(e);
		}
     ...
     */
	@Override
	public List<LiveRoom> selectAllLiveRoomSorted(LiveRoom liveRoom) throws KBaException {
		List<LiveRoom> liveRooms = null;
		try {
			liveRooms = liveRoomDao.queryAllSorded(liveRoom);
			if(liveRooms==null) return null;
			if(liveRooms.size()<5){
				return liveRooms;
			}else {
				return liveRooms.subList(0, 5);
			}
		} catch (SQLException e) {
			throw new KBaException(e);
		}
